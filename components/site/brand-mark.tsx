...
export function BrandMark({ settings }: BrandMarkProps) {
  return (
    <Link href="/" className="flex min-w-0 items-center gap-3" aria-label={`${settings.organizationName} home`}>
      {settings.logoPrimary ? <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white p-1 shadow-sm"><Image src={settings.logoPrimary} alt={`Logo ${settings.shortName}`} width={44} height={44} className="h-full w-full object-contain" priority /></span> : <span className="brand-mark brand-mark-orange" aria-hidden="true">AF</span>}
